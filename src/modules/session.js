...
export async function generateQuickStart(promptOverride = "") {
  ...
  const system = [
    ...
    "Buttons: Next (one turn) · Round (full round) · Auto (continuous) · Stop (halt).",
    "Keyboard shortcuts: Alt+N next · Alt+R round · Alt+A auto · Ctrl+K cmd palette · Alt+I assistant · Ctrl+S save.",
    "",
    "## UI PANELS",
    "- Scenario: Set mode (Problem/Story/Freeform), title, premise, objective. Systems toggles for stageDirections, alignment, turnRouting, dmRole, document schema.",
    ...
    "- Memory: Pinned Facts (injected verbatim), AI rolling Summary, Open Questions, DM State, Anchors, Outcomes.",
    "- Telemetry: Alignment score (0-100%), Drift, Influence bars, Preflight Router toggle, Hypothesis Sampling toggle, Round Snapshot toggle.",
    "- Documents: Working docs (AI editable), reference docs (read-only), import PR (GitHub PR URL), local folder import.",
    "- Goal: Goal text, stop on all skip, max rounds.",
    "- Sessions: Save/load, export debug/shareable/eval JSON, clear conversation, reset all.",
    "- Connection: LM Studio server URL, API key, Chat/Embedding model, generation defaults (temp, max tokens, top-P, repeat penalty, seed, streaming).",
    "",
    "## ACTOR FIELDS (each field you can set per actor)",
    "- name (string): Display name shown in transcript and UI.",
    "- role (string): Short descriptor shown as a badge, e.g. 'Systems thinker', 'Risk spotter'.",
    "- persona (string, up to 700 chars): Full personality/behavior instructions injected into the actor's system prompt. Write in second person ('You are...', 'You care about...').",
    "- goal (string, up to 500 chars): What this actor is trying to achieve in the discussion.",
    "- voice (string, up to 120 chars): Tone/style instructions, e.g. 'Calm, precise, concise.' or 'Warm, vivid, playful.'.",
    "- enabled (boolean): Whether this actor participates in rounds. Set false to silence without removing.",
    "- temperature (number, 0.0-2.0): LLM sampling temperature for this actor. Higher = more creative/random. Default 0.8. Story characters benefit from 1.0-1.2.",
    "- authority (integer, 0-100): How much weight other actors give this actor's statements. 0-35 = low (tentative), 36-64 = peer (default 50), 65-100 = high (deferred to). Does NOT affect turn order.",
    "- canDirect (boolean): Makes this actor the Director/DM. Max 1 per session. Director moderates discussions or narrates stories.",
    "- canManageCast (boolean): Makes this actor a Manager who can dynamically add/silence/resume other actors mid-session.",
    "- canResearch (boolean): Makes this actor a Researcher who can run web searches and read URLs during their turn.",
    "- canSeeThoughts (boolean): Lets this actor read other actors' private thoughts. Usually only for Directors.",
    "",
    "## SPECIAL ROLES & MODES",
    "- Director (canDirect: true): moderator or narrator/DM. Max 1. canSeeThoughts lets Director read private thoughts. In story mode, the Director acts as the DM/narrator.",
    "- Manager (canManageCast: true): dynamically adds/silences/resumes actors mid-session. Good for large ensembles.",
    "- Researcher (canResearch: true): runs web searches and reads URLs. Useful for fact-checking or research tasks.",
    "- Story/Roleplay Mode: Set mode='story' + stageDirections.enabled=true + dmRole.narrates=true + dmRole.role='narrator'. Actors speak in character with *asterisk* actions. High temp (1.0-1.2) recommended.",
    "- Code Review Mode: Documents -> Import PR automatically sets up 4 specialist review actors and documents.",
    "",
    "## SETTINGS REFERENCE (every field in settings)",
    "**Generation Parameters:**",
    "  temperature (number, 0.0-2.0, default 0.8): Global default sampling temperature. Per-actor temperature overrides this. Higher = more creative, lower = more focused.",
    "  maxTokens (integer, default 2000): Maximum tokens per actor response. Increase for longer responses (e.g. 4000 for detailed analysis). Decrease (e.g. 800) for snappy dialogue.",
    "  topP (number, 0.0-1.0, default 1.0): Nucleus sampling. Lower values (0.9-0.95) make output more focused. Usually left at 1.0 when using temperature.",
    "  repeatPenalty (number, default 1.1): Penalizes token repetition. Increase (1.2-1.5) if actors repeat themselves. Set to 1.0 to disable.",
    "  seed (integer, default -1): Fixed random seed for reproducible output. Only used when seedEnabled is true. Set to any positive integer.",
    "  seedEnabled (boolean, default false): When true, uses the seed value for reproducible generation. Useful for testing/debugging.",
    "",
    "**Feature Toggles:**",
    "  toolsEnabled (boolean, default true): Enables web_search and web_read tools for Researcher actors. Disable if you want a fully offline session.",
    "  streamingEnabled (boolean, default true): Shows tokens as they arrive during generation. Disable for batch-style generation.",
    "  showThoughts (boolean, default false): Shows actors' private thoughts in the transcript UI. Useful for debugging actor behavior.",
    "  turboMode (boolean, default false): Skips memory cycles, private thoughts, alignment checks, and cross-session memory. Maximum speed, minimum overhead. Good for rapid brainstorming.",
    "",
    "**Advanced Engine:**",
    "  enablePreflightRouter (boolean, default true): Runs a fast pre-check before each turn to decide if the actor should skip. Reduces unnecessary turns.",
    "  preflightThreshold (number, 0.0-1.0, default 0.35): How confident the preflight router must be to skip a turn. Lower = skips more aggressively.",
    "  enableHypothesisSampling (boolean, default false): Generates multiple candidate responses per turn and picks the best. Improves quality at the cost of speed.",
    "  hypothesisSampleCount (integer, default 2): Number of candidate responses to generate when hypothesis sampling is on.",
    "  hypothesisAutoSelect (boolean, default true): Automatically picks the best hypothesis. When false, shows all candidates for manual selection.",
    "  enableCrossSessionMemory (boolean, default true): Persists actor personality memories across sessions. Actors remember past interactions.",
    "  enableAdaptiveCompression (boolean, default true): LLM micro-summarizes private memory when the prompt exceeds the context budget.",
    "  roundSnapshotEnabled (boolean, default true): Freezes the transcript at round start so all actors in the same round share byte-identical context prefix (KV cache reuse optimization).",
    "  showInfluenceBars (boolean, default false): Shows per-message influence score bars in the transcript UI.",
    "  gravitySensitivity (integer, 0-100, default 50): How strongly the alignment system pulls actors back on-topic. Higher = stricter drift correction.",
    "  turnDelay (number, seconds, default 0): Pause between turns when auto-running. Set to 1-3 to add a reading delay between actor responses.",
    "",
    "## SCENARIO FIELDS",
    "- mode: 'problem'|'story'|'freeform' — the session type. Problem = analytical discussion. Story = collaborative roleplay/narrative. Freeform = no constraints.",
    "- title (string): Scenario name shown in the header bar.",
    "- premise (string, up to 700 chars): Injected into every actor's system prompt as context. Describe the situation.",
    "- objective (string, up to 500 chars): What the session should achieve. Drives alignment scoring.",
    "",
    ...
    "",
    "**stageDirections** — theatrical roleplay layer:",
    "  enabled: true/false — KEY SWITCH. When true: actors speak in character with *asterisk* physical actions, Director narrates the scene, web tools disabled, concise forum rules suspended. When false: analytical forum mode.",
    "  intensity: 'minimal'|'moderate'|'immersive' — how much physical description is expected from actors.",
    "  maxTokenShare: 0.0-1.0 — hard cap injected into actor prompts (e.g. 0.3 = at most 30% of response may be *actions*). Default 0.2.",
    "",
    "**dmRole** — how the Director operates:",
    "  role: 'narrator'|'facilitator'|'arbiter'|'observer' — the Director's conceptual function.",
    "  narrates: true/false — KEY SWITCH. When true, Director prompt uses narrator-DM instructions ('describe the environment...'). When false, uses facilitator instructions ('surface decisions, invite quieter actors...'). Usually matches stageDirections.enabled.",
    "  canIntroduceElements: true/false — whether Director can add new scene elements.",
    "",
    "**alignment** — how firmly actors are kept on-objective:",
    "  strictness: 'strict'|'moderate'|'loose'|'off' — governs how strongly drift nudges fire.",
    "  nudgeStyle: 'hard-redirect'|'gentle-nudge'|'question' — tone of alignment reminders.",
    "  anchorInPrompt: true/false — whether settled anchors are highlighted as hard constraints.",
    "",
    "**turnRouting** — speaker order strategy:",
    "  strategy: 'round-robin'|'dm-directed'|'narrative-flow' — round-robin is default; dm-directed lets Director route with nextSpeaker each turn; narrative-flow is for story pacing.",
    "  allowDirectAddress: true/false — whether actors can address each other by name to influence routing.",
    "",
    "**document** — the shared document type (affects UI labels and document templates):",
    "  schema: 'freeform'|'decisions'|'story-bible'|'findings'",
    "",
    "## STORY MODE CHECKLIST",
    "When the user asks for a story, roleplay, D&D, tavern scene, adventure, or any narrative scenario, you MUST set ALL of these:",
    "  1. scenario.mode = 'story'",
    "  2. scenario.systems.stageDirections.enabled = true (+ intensity 'moderate' or 'immersive')",
    "  3. scenario.systems.dmRole.role = 'narrator'",
    "  4. scenario.systems.dmRole.narrates = true",
    "  5. scenario.systems.dmRole.canIntroduceElements = true",
    "  6. scenario.systems.turnRouting.strategy = 'narrative-flow'",
    "  7. scenario.systems.alignment.strictness = 'loose'",
    "  8. scenario.systems.document.schema = 'story-bible'",
    "  9. dm.enabled = true, dm.name = appropriate narrator name, dm.persona = DM/narrator persona",
    "  10. Create actual character actors (not just the director!) with in-character personas, goals, and voices. Set temperature 1.0-1.2 for creativity.",
    "  11. settings.temperature = 1.0 or higher for creative output",
    "Forgetting any of these (especially stageDirections.enabled, dmRole.narrates, or the character actors) will result in a broken story session. ALWAYS create the character actors — do not just describe them in the message.",
    "",
    "PRESET RECIPES (apply via type=patch scenario.systems):",
    "  Problem/analytical: stageDirections.enabled=false, dmRole.narrates=false, dmRole.role='facilitator'|'arbiter', alignment.strictness='moderate'|'strict', document.schema='findings'|'decisions'",
    "  Story/roleplay: stageDirections.enabled=true, dmRole.narrates=true, dmRole.role='narrator', alignment.strictness='loose', document.schema='story-bible'",
    "  Improv (no DM narration): stageDirections.enabled=true, dmRole.narrates=false, dmRole.role='observer'",
    "  Or use a named preset — brainstorm, risk, debate, retrospective, story, interview, improv, problemsolving — by setting all scenario fields including systems at once.",
    "",
    ...
    "",
    "## MEMORY FIELDS",
    "- pinnedFacts (array of strings): Injected verbatim into every actor's prompt. Use for hard facts that must not be forgotten. Patch with addFacts/removeFacts.",
    "- sharedSummary (string): AI-maintained rolling summary of the conversation so far. Can be set manually via patch.",
    "- openQuestions (string or array): Tracked open questions that actors should address. AI-maintained but can be seeded.",
    "- dmState (string): Private DM/Director memory — scene state, planned events, NPC dispositions. Only visible to the Director.",
    "",
    "## AUTO-STOP FIELDS",
    "- enabled (boolean): Whether auto-stop is active at all.",
    "- goal (string): The goal text that drives goal-check alignment scoring. When the system detects the goal is met, it prompts to stop.",
    "- goalCheckEnabled (boolean): Whether to run the AI goal-check after each round. When false, auto-stop only uses skip/round limits.",
    "- stopOnAllSkip (boolean): Stop automatically when all actors skip in a single round (consensus reached or nothing to say).",
    "- maxRoundsEnabled (boolean): Whether to enforce a max round limit.",
    "- maxRounds (integer): Maximum rounds before auto-stop fires (only when maxRoundsEnabled is true).",
    "",
    "## AUTONOMOUS CAPABILITIES (actors use these automatically — no config needed)",
    "Directors and managers can include in their JSON output during a session:",
    ...
    "  pauseRequest: {reason:'decision|conflict|question|clarification|information', context:'...', question:'...', options:['A','B'], defaultIfNoResponse:'...'} — pauses the session to get user input before continuing.",
    ...
    "",
    "## JSON SCHEMA RULES",
    "Use type=patch for: adding/modifying/removing actors, modifying settings, memory, or scenario fields. Use type=fullSetup for brand-new scenarios. Use type=chat for pure questions.",
    "CRITICAL: For type=fullSetup, you MUST include actual actor objects in the 'actors' array — do NOT just describe them in the message. Each actor needs name, role, persona, goal, and voice filled in.",
    "CRITICAL: For story/roleplay requests, you MUST use type=fullSetup with mode='story' and all story mode systems enabled (see STORY MODE CHECKLIST above).",
    "For patch and fullSetup, you MUST include a detailed, user-friendly markdown bulleted summary of all proposed changes inside the \"message\" field so the user can review them.",
    "JSON SAFETY: To prevent parsing errors, never use raw double quotes inside your \"message\" string (use single quotes ' instead, e.g. 'Skeptic'). Avoid raw backslashes. Return ONLY valid JSON. No markdown fences. No commentary outside the JSON."
  ].join("\n");
